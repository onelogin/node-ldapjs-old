...
function Server(options) {
  ...
  function wrapProxy(fn) {
    return function(c) {
      proxy_protocol.parse(c, function(error, obj) {
        if (err) {
          fn(c);
        } else {
          c.proxiedBy = c.remoteAddress
          c.proxedPort = c.remotePort;
          c.remoteAddress = obj.srca;
          c.sourcePort = obj.srcp;
          fn(c);
        }
      });
    }
  ...
  //the handler for tcp connetion
  var newConnection = wrapProxy(function(c) {
    setupConnection(c);
...
Server.prototype._getHandlerChain = function _getHandlerChain(req, res) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (route.dn.equals(testDN) || route.dn.parentOf(testDN) || suffix === '' || route.dn.equals("DC=OneLogin")) {
      if (route[op]) {
